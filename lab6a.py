...
class Student:
    ...
    def displayStudent(self):
        return 'Student Name: ' + self.name + '\n' + 'Student Number: ' + str(self.number) # we must add str() to fixed TypeError trying to concatenate (join together) strings using the + operator.
    ...
    def displayGPA(self):
        if not self.courses:                             #for fixing ZeroDivisionError we need to add a check to see if the student has any courses before performing the division.
    ...
    def displayCourses(self):
        passed = [] #creates an empty list to store the courses the student passed
        for c, g in self.courses.items(): #Iterate through the courses and grades

            if g:                         #Check if the grade is not 0.0 
                passed.append(c)         #If the course was passed, add its name to the list
        return passed
